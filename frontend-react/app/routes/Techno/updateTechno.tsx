...
export async function clientLoader({ params }: Route.ClientLoaderArgs) {
  ...
  const response = await fetch(
    `${import.meta.env.VITE_BACKEND_API_URL}/technos/${params.id}`,
    {
      headers: {
  ...
  const techno: TechnoResponse = await response.json();
...
export async function clientAction({ request }: Route.ClientActionArgs) {
  ...
  if (formData.get("intent") === "update") {
    const response = await fetch(
      `${import.meta.env.VITE_BACKEND_API_URL}/technos/${formData.get("id")}`,
      {
        method: "PATCH",
        headers: {
  ...
  return (
    <Fragment>
      ...
      <h1 className="text-3xl font-bold text-gray-800 mb-6">
        Modification de la technologie
      </h1>
